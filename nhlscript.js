    ...
    fetch("https://statsapi.web.nhl.com/api/v1/schedule/games/?sportId=1").then(function(response) {
        response.json().then(function(json) {
            ...
            if (json.totalGames == 0) {
                active.innerHTML = `
                    <h4 style="color: rgb(4, 30, 66)">There are no NHL games today</h4>                         
                    `;
                } else {
                active.innerHTML = `
                    <h4 span style="color: rgb(4, 30, 66)">There are <span style="color: rgb(187, 0, 33)">${json.dates[0].totalGames}</span> NHL games today, <span style="color: rgb(187, 0, 33)">${json.dates[0].totalGamesInProgress}</span> in progress or starting soon</h4>                         
                    `;
                }
